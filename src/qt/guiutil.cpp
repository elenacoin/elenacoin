...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("elenacoin"))
        return false;
...
{
    // Convert elenacoin:// to elenacoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("elenacoin://"))
    {
        uri.replace(0, 12, "elenacoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ElenaCoin.lnk";
}
...
{
    return GetAutostartDir() / "elenacoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ElenaCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  elenacoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("ElenaCoin-Qt"));
