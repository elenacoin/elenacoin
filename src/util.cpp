...
#else
    const char* pszModule = "elenacoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ElenaCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ElenaCoin
    // Mac: ~/Library/Application Support/ElenaCoin
    // Unix: ~/.elenacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ElenaCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "elenacoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "elenacoind.pid"));
